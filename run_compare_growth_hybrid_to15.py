...
=================================
Run and compare the adaptive growing-degree and hybrid local/global approaches
up to degree 15.
...
import adaptive_uvp_degree_growth as growth
import adaptive_uvp_hybrid_local_global as hybrid


OUT_DIR = "poly_saves"

GROWTH_BASENAME = "adaptive_uvp_degree_growth_to15"
HYBRID_BASENAME = "adaptive_uvp_hybrid_local_global_to15"

GROWTH_CSV = os.path.join(OUT_DIR, GROWTH_BASENAME + ".csv")
HYBRID_CSV = os.path.join(OUT_DIR, HYBRID_BASENAME + ".csv")

PLOT_RMSE_TIME = os.path.join(OUT_DIR, "growth_vs_hybrid_to15_rmse_time.png")
PLOT_SCORE_TIME = os.path.join(OUT_DIR, "growth_vs_hybrid_to15_score_time.png")
PLOT_RMSE_SWEEP = os.path.join(OUT_DIR, "growth_vs_hybrid_to15_rmse_sweep.png")
PLOT_SCORE_SWEEP = os.path.join(OUT_DIR, "growth_vs_hybrid_to15_score_sweep.png")
PLOT_BASIS_TIME = os.path.join(OUT_DIR, "growth_vs_hybrid_to15_basis_time.png")
...
def make_plots():
    ...
    series = {
        "growth": read_csv(GROWTH_CSV, "growth"),
        "hybrid": read_csv(HYBRID_CSV, "hybrid"),
    }
    for label, rows in series.items():
        if not rows:
    ...
    plot_metrics(
        ...
        "elapsed_seconds",
        ["score"],
        "Growth vs Hybrid Score by Time",
        "Score",
        PLOT_SCORE_TIME,
    )
    plot_metrics(
        ...
        "global_sweep",
        ["u_rmse", "v_rmse", "p_rmse"],
        "Growth vs Hybrid RMSE by Sweep",
        "RMSE",
        PLOT_RMSE_SWEEP,
    )
    plot_metrics(
        ...
        "global_sweep",
        ["score"],
        "Growth vs Hybrid Score by Sweep",
        "Score",
        PLOT_SCORE_SWEEP,
    )
...
def main():
    ...
    if not args.plot_only:
        ...
        #    print("\nRunning growing-degree experiment to degree 15")
        #    growth.main()
        if not args.skip_hybrid:
            print("\nRunning hybrid local/global experiment to degree 15")
            hybrid.main()
