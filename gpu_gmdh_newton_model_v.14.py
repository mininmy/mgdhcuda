...
class GMDHTrainerGPU:
    ...
    def _train_and_eval_layer(self, X, y, all_pg_polys):
        ...
        R_accum = cp.zeros((n_sys, 6, 6), dtype=cp.float64)
        c_accum = cp.zeros((n_sys, 6),    dtype=cp.float64)
        bsq_acc = cp.zeros(n_sys,         dtype=cp.float64)
        ...
        for start in range(0, n_total, self.chunk_size):
            ...
            for sub_s in range(0, curr_n, self.qr_sub_size):
                ...
                for ss in range(0, n_sys, self.jac_sys_chunk):
                    ...
                    R_accum[ss:se], c_accum[ss:se] = self._tsqr_update(
                        R_accum[ss:se], c_accum[ss:se], J_sub, b_sub, self.qr_sys_chunk)
                    bsq_acc[ss:se] += (b_sub * b_sub).sum(axis=1)
                    del J_sub, b_sub
            del phi_all, dt_all, grad_all, lap_all, pg_all, y_true
            cp.get_default_memory_pool().free_all_blocks()

        return self._solve_from_qr(
            R_accum, c_accum, n_total,
            k_map_cpu, s_arr, kl_arr, ic_arr, kr_arr,
            X, y, all_pg_polys, s_gpu, kr_gpu, ic_gpu)
    ...
    # ------------------------------------------------------------------
    @staticmethod
    def _tsqr_update(R_accum, c_accum, J_sub, b_sub, sys_chunk=256):
        n_sys = R_accum.shape[0]
        R_new = cp.empty_like(R_accum); c_new = cp.empty_like(c_accum)
        for ss in range(0, n_sys, sys_chunk):
            se = min(ss + sys_chunk, n_sys)
            M   = cp.concatenate([R_accum[ss:se], J_sub[ss:se]], axis=1)
            rhs = cp.concatenate([c_accum[ss:se], b_sub[ss:se]], axis=1)
            Q, R = cp.linalg.qr(M, mode='reduced')
            c = (Q.transpose(0, 2, 1) @ rhs[:, :, None])[:, :, 0]
            R_new[ss:se] = R; c_new[ss:se] = c
        return R_new, c_new

    # ------------------------------------------------------------------
    def _solve_from_qr(self, R_accum, c_accum, n_total,
                       k_map_cpu, s_arr, kl_arr, ic_arr, kr_arr,
                       X, y, all_pg_polys, s_gpu, kr_gpu, ic_gpu):
        n_sys = R_accum.shape[0]; n_dim = self.n_vars - 1
        ic_clamped = cp.minimum(ic_gpu, cp.int32(n_dim - 1))

        U, s, Vh = cp.linalg.svd(R_accum, full_matrices=False)
        s_max  = s.max(axis=-1, keepdims=True)
        s_inv  = cp.where(s > self.svd_rcond * s_max, cp.float64(1.0)/s, cp.float64(0.0))
        UTc        = U.transpose(0, 2, 1) @ c_accum[:, :, None]
        alphas_gpu = (Vh.transpose(0, 2, 1) * s_inv[:, None, :]) @ UTc
        alphas_gpu = alphas_gpu[:, :, 0]

        physics_ss = cp.zeros(n_sys, dtype=cp.float64)
        alpha_col  = alphas_gpu[:, :, None]
        ...
        for start in range(0, n_total, self.chunk_size):
            ...
            phi_all, dt_all, grad_all, lap_all = \
                self._precompute_all_models_fast(X_c, self.current_models)
            pg_all = cp.zeros((len(self.current_models), n_dim, curr_n), dtype=cp.float64)
            for m_idx, pg_model in enumerate(all_pg_polys):
                for d in range(n_dim):
                    if pg_model[d].exponents.shape[0] > 0:
                        pg_all[m_idx, d] = pg_model[d].evaluate(X_c)
            ...
            for sub_s in range(0, curr_n, self.qr_sub_size):
                ...
                for ss in range(0, n_sys, self.jac_sys_chunk):
                    ...
                    J_sub, b_sub = self._compute_jacobian_rows_vectorized(
                        ...
                        s_gpu[ss:se], kr_gpu[ss:se],
                        ic_gpu[ss:se], ic_clamped[ss:se], n_dim)
                    J_alpha   = (J_sub @ alpha_col[ss:se])[:, :, 0]
                    corrected = J_alpha - b_sub
                    physics_ss[ss:se] += (corrected**2).sum(axis=1)
                    del J_sub, b_sub, J_alpha, corrected
            del phi_all, dt_all, grad_all, lap_all, pg_all, y_true
            cp.get_default_memory_pool().free_all_blocks()

        rmse_all   = cp.sqrt(physics_ss / max(n_total, 1))
        alphas_cpu = alphas_gpu.get(); rmse_cpu = rmse_all.get()
        candidates = []
        for sys_idx in range(n_sys):
            candidates.append({'s': int(s_arr[sys_idx]), 'k': int(kr_arr[sys_idx]),
                                'i': int(ic_arr[sys_idx]), 'alpha': alphas_cpu[sys_idx],
