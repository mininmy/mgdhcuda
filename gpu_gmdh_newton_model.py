...
class GMDHTrainerGPU:
    ...
    def fit(self, X_cpu, y_target_cpu, n_layers=11):
        ...
        for layer in range(n_layers):
            ...
            avg_rmse   = sum(c['err'] for c in winners) / max(len(winners), 1)
            ...
            # Stop when the average RMSE across the retained top models stops
            # improving — a more robust signal than the single-best RMSE,
            # since one lucky candidate can mask deterioration of the pool.
    ...
    def _assemble(self, winners):
        ...
        # winners are ordered [top_models for comp0, top_models for comp1, ...],
        # sorted best-first within each component.  The best poly for component
        # ic is therefore the first poly in that component's block.
        n_per_comp = len(new_layer_polys) // self.n_comp
        self.best_model = [new_layer_polys[ic * n_per_comp] for ic in range(self.n_comp)]
