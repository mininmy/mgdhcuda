...
class PhysicsAwareGMDH:
    ...
    def _to_gpu(self, **arrays):
        ...
        _sync_gpu()
        return out
    def _least_squares_step(self, weights, group_weights=None):
        """
        Residual-projection step.
        For each (component, s, k) pair:
        - build correction basis Phi
        - project physics residuals onto Phi
        ...
        # --------------------------------------------------
        # A. Evaluate fields on CPU (identical to old logic)
        # --------------------------------------------------
        ...
        # --------------------------------------------------
        # B. Move to GPU (large tensors only)
        # --------------------------------------------------
        ...
        # --------------------------------------------------
        # C. Shared model state (passed into Dask tasks)
        # --------------------------------------------------
        ...
        model_state = {
            ...
            "u_tensor_gpu": gpu["u_tensor"],
            "u_grads_tensor_cpu": u_grads_tensor_cpu,
            "u_grads_tensor_gpu": gpu["u_grads"],
            "u_dt_gpu": gpu["u_dt"],
            "u_laplace_gpu": gpu["u_laplace"],
            ...
            for s in range(n_models):
                for k in range(n_models):
                    if s == k:
                        continue
                    tasks.append(
                        evaluate_model_pair_delayed(
                            comp_idx, s, k, model_state
                        )
                    )

        # --------------------------------------------------
        # E. Solve all correction problems
        # --------------------------------------------------
        ...
        for res in results:
            ...
            corrections.append((comp_idx, s_idx, k_idx, coeffs))
            error_vectors.append(errors)
            y_preds.append(y_pred)

        return corrections, error_vectors, y_preds

    def _least_squares_step_newton(self, pressure_poly, weights, group_weights=None):
        """
    ...
    def fit(self, X, y):
        """
        Physics-driven GMDH training loop.
        Each layer performs a residual-projection update.
        """
        ...
        self.y_cpu = np.asarray(y[:, :limit])

        X_gpu_test = cp.asarray(self.X_cpu)
        y_gpu_test = cp.asarray(self.y_cpu).T
        ...
        if self.current_layer == 0:
            self.models = [self._initialize_first_layer(
                n_components=self.y_cpu.shape[0]
            )]

            self.pressures = []
            velocities_list = list(map(list, zip(*self.models[0])))
            for velocities in velocities_list:
                p = compute_pressure_from_polynomials(
            ...
            for comp_models in self.models[-1]:
                for poly in comp_models:
                    poly.group_weights = {"data": 1.0, "incomp": 1.0, "momentum": 1.0}
        ...
        while self.current_layer < self.max_layer:
            ...
            # ---- residual projection ----
            corrections, errors, preds = self._least_squares_step(
            ...
            # --------------------------------------------------
            # D. Apply corrections → candidate models
            # --------------------------------------------------
            ...
            for idx, (comp_idx, s, k, coef) in enumerate(corrections):
                new_poly = self.models[-1][comp_idx][s].combine_with_gpu(
                    self.models[-1][comp_idx][k], *coef
                )

                velocities = [
                    self.models[-1][j][s] if j != comp_idx else new_poly
                    for j in range(self.y_cpu.shape[0])
                ]

                candidate_velocities.append(velocities)

                phys = gmdh_error_lightweight_eval(
                    X_gpu_test,
                    y_gpu_test,
                    velocity_polys=velocities,
                    pressure_poly=self.pressures[s],
                    nu=self.viscosity,
                    return_components=True,
                )

                physics_errors.append(phys["total"])
                error_components.append(phys)

            # --------------------------------------------------
            # E. Select best candidates
            # --------------------------------------------------

            physics_errors = np.asarray(physics_errors)
            best_idx = np.argsort(physics_errors)[:self.top_models]

            candidate_velocities = [candidate_velocities[i] for i in best_idx]

            # --------------------------------------------------
            # F. Update group weights
            # --------------------------------------------------
            ...
            for i in best_idx:
                e = error_components[i]
                wd = 1.0 / (e["data"] + eps)
                wi = 1.0 / (e["incompressibility"] + eps)
                wm = 1.0 / (e["navier_stokes"] + eps)
                s = wd + wi + wm
            ...
            for gw, velocities in zip(selected_group_weights, candidate_velocities):
                for poly in velocities:
                    poly.group_weights = gw

            # --------------------------------------------------
            # G. Recompute pressure
            # --------------------------------------------------

            self.pressures = []
            for velocities in candidate_velocities:
                p = compute_pressure_from_polynomials(
                    velocities, viscosity=self.viscosity
                )
                self.pressures.append(p)
            ...
            self.models.append(list(map(list, zip(*candidate_velocities))))
