...
def _collect_refit_basis(prev_poly, new_poly, new_scale, n_vars,
                         max_terms=MAX_REFIT_TERMS):
    scores = {}
    if prev_poly is not None:
        _score_poly_terms(scores, prev_poly, 1.0)
    _score_poly_terms(scores, new_poly, new_scale)
...
if __name__ == "__main__":
    ...
    for round_idx in range(MAX_BOOST_ROUNDS):
        ...
        for ic in range(n_comp):
            ...
            refit_polys[ic] = _refit_polynomial(
                X,
                y[ic],
                basis_exps,
                ridge_lambda=REFIT_RIDGE,
            )
            refit_polys[ic]._ensure_cpu_cache()
            cum_pairs[ic] = [(1.0, refit_polys[ic])]
            print(f"  Refit component {ic}: {basis_exps.shape[0]} monomials")

        cum_pred = _predict_polys(refit_polys, X)
