...
class GMDHTrainerGPU:
    ...
    def fit(self, X_cpu, y_target_cpu, n_layers=11,
        ...
        for layer in range(n_layers):
            ...
            t_start = time.time()
            candidates, current_rmse = self._train_and_eval_layer(
                X_tr, y_tr,
...
if __name__ == "__main__":
    ...
    for _l in range(N_LAYERS):
        ...
        sched.append({
            'top_models':    max(20, int(100 * (1.0 - 0.6 * frac))),   # 100→40
            'corr_threshold': 0.90 + 0.095 * frac,                     # 0.90→0.995
