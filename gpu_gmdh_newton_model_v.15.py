...
class GMDHTrainerGPU:
    ...
    def _train_and_eval_layer(self, X, y, all_pg_polys):
        ...
        # Normal equations with separate constant estimation.
        # Constant col (col 0) is NOT included in XTX/XTy.
        # Instead sum_b = Σ b  → alpha_0 = sum_b/N  (OLS intercept).
        # Decouples alpha_0 from the polynomial columns, preventing
        # near-collinearity between col0=ones and col4=S² when S has
        # a constant offset (condition number drops from ~1e16 to ~1e5).
        XTX   = cp.zeros((n_sys, 5, 5), dtype=cp.float64)  # J5^T J5
        XTy   = cp.zeros((n_sys, 5),    dtype=cp.float64)  # J5^T b
        sum_b  = cp.zeros(n_sys,         dtype=cp.float64)  # Σ b  → alpha_0
        sum_J5 = cp.zeros((n_sys, 5),    dtype=cp.float64)  # Σ J5 columns
        ...
        for start in range(0, n_total, self.chunk_size):
            ...
            for sub_s in range(0, curr_n, self.qr_sub_size):
                ...
                for ss in range(0, n_sys, self.jac_sys_chunk):
                    ...
                    # J5_sub = cols 1-5 (skip col 0 which is always zero)
                    J5_sub = J_sub[:, :, 1:]   # [c, B, 5]
                    XTX[ss:se]   += J5_sub.transpose(0, 2, 1) @ J5_sub
                    XTy[ss:se]   += (J5_sub.transpose(0, 2, 1) @ b_sub[:, :, None])[:, :, 0]
                    sum_b[ss:se]   += b_sub.sum(axis=1)
                    sum_J5[ss:se]  += J5_sub.sum(axis=1)   # [c, 5]
                    bsq[ss:se]     += (b_sub * b_sub).sum(axis=1)
                    del J_sub, b_sub
            del phi_all, dt_all, grad_all, lap_all, pg_all, y_true
            cp.get_default_memory_pool().free_all_blocks()

        # ---- Solve: OLS intercept + 5-col polynomial system ----
        # alpha_0 = mean(b) estimated independently.
        # Center XTy: XTy_c = J5^T (b - alpha_0) = XTy - alpha_0 * sum_J5
        # This decouples the constant from polynomial cols, reducing
        # condition number from ~1e16 to ~1e5 when S has a constant offset.
        alpha_0 = sum_b / max(n_total, 1)   # [n_sys]

        XTX   = (XTX + XTX.transpose(0, 2, 1)) * cp.float64(0.5)
        XTy_c = XTy - alpha_0[:, None] * sum_J5

        U, s, Vh = cp.linalg.svd(XTX, full_matrices=False)
        s_max  = s.max(axis=-1, keepdims=True)
        s_inv  = cp.where(s > self.svd_rcond * s_max,
                          cp.float64(1.0) / s, cp.float64(0.0))
        alpha_rest = (Vh.transpose(0, 2, 1) * s_inv[:, None, :]) @ \
                     (U.transpose(0, 2, 1) @ XTy_c[:, :, None])
        alpha_rest = alpha_rest[:, :, 0]   # [n_sys, 5]

        # Combine: alpha = [alpha_0, alpha_rest[0..4]]
        alphas_gpu = cp.concatenate([alpha_0[:, None], alpha_rest], axis=1)  # [n_sys, 6]

        # RMSE identity (no second pass):
        #   ||b_c - J5 alpha_rest||^2
        #   = bsq - N*alpha_0^2 - 2*alpha_rest^T XTy_c + alpha_rest^T XTX alpha_rest
        aXTy_c = (alpha_rest[:, None, :] @ XTy_c[:, :, None])[:, 0, 0]
        aXTXa  = (alpha_rest[:, None, :] @ XTX @ alpha_rest[:, :, None])[:, 0, 0]
        residual_ss = cp.maximum(
            bsq - cp.float64(n_total) * alpha_0**2
            - np.float64(2.0) * aXTy_c + aXTXa,
            cp.float64(0.0))
